...
- `database` (*Read-Only*) (Set Of Object). ~> Deprecated! To manage databases, please switch to using a separate resource type `yandex_mdb_postgresql_database`.
  - `extension` . 
    - `name` . 
    - `version` . 
  - `lc_collate` . 
...
- `user` (*Read-Only*) (List Of Object). ~> Deprecated! To manage users, please switch to using a separate resource type `yandex_mdb_postgresql_user`.
  - `conn_limit` . 
  - `grants` . 
  - `login` . 
  - `name` . 
