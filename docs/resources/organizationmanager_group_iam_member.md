...
- `group_id` (String) The ID of the compute `group` to attach the policy to.
...
- `role` (String) The role that should be assigned. Only one yandex_organizationmanager_group_iam_member can be used per role.
