...
Allows creation and management of a single binding within IAM policy for an existing `dns_firewall`.

~> **Warning:** This resource is authoritative for the given `role` on the target `dns_firewall` and manages the complete set of its members. When you change or delete `yandex_dns_firewall_iam_binding`, the `role` may be removed from other subjects on the `dns_firewall` as well — including subjects granted outside of this resource (via the corresponding `*_iam_member` resource, the management console, CLI or API). Those subjects are not tracked in the Terraform state, so a plain `terraform plan` does not list them. Be careful.
