...
func testAccDataSourceMDBMgUserCheckResourceIDField(resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		expectedResourceId := utils.ConstructResourceId(rs.Primary.Attributes["cluster_id"], rs.Primary.Attributes["name"])
...
func testAccCheckMDBMongoDBUserDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		clusterId, userName, err := utils.DeconstructResourceId(rs.Primary.ID)
		if err != nil {
			return err
		}
