...
package yandex_dataproc_cluster_iam_binding
	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"role": schema.StringAttribute{
				MarkdownDescription: "The role that should be assigned. Only one yandex_dataproc_cluster_iam_binding can be used per role.",
				Required:            true,
			},
...
func (r *IAMUpdater) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "yandex_dataproc_cluster_iam_binding"
}
...
func (u *IAMUpdater) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if len(binding) == 0 {
		tflog.Debug(ctx,
			fmt.Sprintf(
				"Resource yandex_dataproc_cluster_iam_binding '%s' is missing or deleted, marking policy binding as deleted",
				u.clusterId,
			),
		)
		return
...
// Because yandex_dataproc_cluster_iam_binding is authoritative for a single role and applies changes via SetAccessBindings
...
func (u *IAMUpdater) ModifyPlan(ctx context.Context, req resource.ModifyPlanRequest, resp *resource.ModifyPlanResponse) {
	...
	if err != nil {
		...
		tflog.Debug(ctx, fmt.Sprintf("ModifyPlan: unable to read access bindings of yandex_dataproc_cluster_iam_binding '%s': %s", u.clusterId, err))
		return
	}
	...
	resp.Diagnostics.AddWarning(
		fmt.Sprintf("Role %q will be removed from %d subject(s) of yandex_dataproc_cluster_iam_binding %q", role.ValueString(), len(removed), u.clusterId),
...
func (u *IAMUpdater) GetResourceIamPolicy(ctx context.Context) (*accessbinding.Policy, error) {
	...
	for {
		md := new(metadata.MD)
		resp, err := dataprocv1sdk.NewClusterClient(u.providerConfig.SDKv2).ListAccessBindings(ctx, &access.ListAccessBindingsRequest{
		...
		if traceHeader := md.Get("x-server-trace-id"); len(traceHeader) > 0 {
			tflog.Debug(ctx, "List yandex_dataproc_cluster_iam_binding trace header", map[string]interface{}{
				"x-server-trace-id": traceHeader[0],
			})
		}
		if traceHeader := md.Get("x-server-request-id"); len(traceHeader) > 0 {
			tflog.Debug(ctx, "List yandex_dataproc_cluster_iam_binding request header", map[string]interface{}{
...
func (u *IAMUpdater) SetResourceIamPolicy(ctx context.Context, policy *accessbinding.Policy) error {
	...
	op, err := dataprocv1sdk.NewClusterClient(u.providerConfig.SDKv2).SetAccessBindings(ctx, req, grpc.Header(md))
	if err != nil {
		return fmt.Errorf("error setting access bindings of yandex_dataproc_cluster_iam_binding '%s': %w", u.clusterId, err)
	}
...
func (u *IAMUpdater) UpdateResourceIamPolicy(ctx context.Context, policy *accessbinding.PolicyDelta) error {
	...
	for i := 0; i < accessbinding.CountBatches(dLen, bSize); i++ {
		...
		op, err := dataprocv1sdk.NewClusterClient(u.providerConfig.SDKv2).UpdateAccessBindings(ctx, req)
		if err != nil {
			return fmt.Errorf("error updating access bindings of yandex_dataproc_cluster_iam_binding '%s': %w", u.clusterId, err)
		}
...
func (u *IAMUpdater) refreshBindingState(ctx context.Context, req accessbinding.Extractable, resp accessbinding.Settable, diag *diag.Diagnostics) {
	...
	if err != nil {
		if accessbinding.IsStatusWithCode(err, codes.NotFound) {
			tflog.Debug(ctx, fmt.Sprintf("Resource yandex_dataproc_cluster_iam_binding '%s' not found, removing from state", u.clusterId))
	...
	tflog.Debug(ctx, fmt.Sprintf("Retrieved access bindings of yandex_dataproc_cluster_iam_binding '%s': %+v", u.clusterId, policy))
...
func (u *IAMUpdater) iamPolicyReadModifySet(ctx context.Context, updater accessbinding.ResourceIamUpdater, modify iamPolicyModifyFunc) error {
	mutexKV.Lock(fmt.Sprintf("yandex_dataproc_cluster_iam_binding-%s", u.clusterId))
	defer mutexKV.Unlock(fmt.Sprintf("yandex_dataproc_cluster_iam_binding-%s", u.clusterId))
	tflog.Debug(ctx, fmt.Sprintf("Retrieving access bindings for yandex_dataproc_cluster_iam_binding '%s'", u.clusterId))

	p, err := updater.GetResourceIamPolicy(ctx)
	if err != nil {
		return err
	}
	tflog.Debug(ctx, fmt.Sprintf("Retrieved access bindings for yandex_dataproc_cluster_iam_binding '%s': %+v\n", u.clusterId, p))

	err = modify(p)
	if err != nil {
		return err
	}
	tflog.Debug(ctx, fmt.Sprintf("Setting access bindings for yandex_dataproc_cluster_iam_binding '%s' to %+v", u.clusterId, p))

	err = updater.SetResourceIamPolicy(ctx, p)
	if err != nil {
		return fmt.Errorf("Error applying access bindings to yandex_dataproc_cluster_iam_binding '%s': %w", u.clusterId, err)
	}
	tflog.Debug(ctx, fmt.Sprintf("Set policy for yandex_dataproc_cluster_iam_binding '%s'", u.clusterId))
