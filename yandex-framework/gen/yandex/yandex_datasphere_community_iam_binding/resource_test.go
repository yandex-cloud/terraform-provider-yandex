package yandex_datasphere_community_iam_binding_test
...
func testAccCheckCommunityIam(resourceName, role string, members []string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		bindings, err := dataspheresdk.NewCommunityClient(config.SDKv2).ListAccessBindings(context.Background(), &access.ListAccessBindingsRequest{
			ResourceId: rs.Primary.ID,
		})
		if err != nil {
			return fmt.Errorf("get access bindings for resource id(%s):  %w", rs.Primary.ID, err)
		}
