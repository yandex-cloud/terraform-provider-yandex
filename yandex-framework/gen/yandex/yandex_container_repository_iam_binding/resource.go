	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"repository_id": schema.StringAttribute{
				MarkdownDescription: "The ID of the `repository` to attach the policy to.",
				Required:            true,
			},
...
// Because yandex_container_repository_iam_binding is authoritative for a single role and applies changes via SetAccessBindings
...
func (u *IAMUpdater) ModifyPlan(ctx context.Context, req resource.ModifyPlanRequest, resp *resource.ModifyPlanResponse) {
	...
	if err != nil {
		...
		tflog.Debug(ctx, fmt.Sprintf("ModifyPlan: unable to read access bindings of yandex_container_repository_iam_binding '%s': %s", u.repositoryId, err))
		return
	}
	...
	resp.Diagnostics.AddWarning(
		fmt.Sprintf("Role %q will be removed from %d subject(s) of yandex_container_repository_iam_binding %q", role.ValueString(), len(removed), u.repositoryId),
...
func (u *IAMUpdater) refreshBindingState(ctx context.Context, req accessbinding.Extractable, resp accessbinding.Settable, diag *diag.Diagnostics) {
	...
	if err != nil {
		if accessbinding.IsStatusWithCode(err, codes.NotFound) {
			tflog.Debug(ctx, fmt.Sprintf("Resource yandex_container_repository_iam_binding '%s' not found, removing from state", u.repositoryId))
	...
	diag.Append(resp.SetAttribute(ctx, path.Root("id"), u.repositoryId+"/"+role.ValueString())...)
}
