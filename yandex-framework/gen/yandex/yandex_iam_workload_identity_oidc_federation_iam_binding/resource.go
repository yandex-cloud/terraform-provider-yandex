	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"federation_id": schema.StringAttribute{
				MarkdownDescription: "The ID of the `federation` to attach the policy to.",
				Required:            true,
			},
...
// Because yandex_iam_workload_identity_oidc_federation_iam_binding is authoritative for a single role and applies changes via SetAccessBindings
...
func (u *IAMUpdater) ModifyPlan(ctx context.Context, req resource.ModifyPlanRequest, resp *resource.ModifyPlanResponse) {
	...
	if err != nil {
		...
		tflog.Debug(ctx, fmt.Sprintf("ModifyPlan: unable to read access bindings of yandex_iam_workload_identity_oidc_federation_iam_binding '%s': %s", u.federationId, err))
		return
	}
	...
	resp.Diagnostics.AddWarning(
		fmt.Sprintf("Role %q will be removed from %d subject(s) of yandex_iam_workload_identity_oidc_federation_iam_binding %q", role.ValueString(), len(removed), u.federationId),
...
func (u *IAMUpdater) refreshBindingState(ctx context.Context, req accessbinding.Extractable, resp accessbinding.Settable, diag *diag.Diagnostics) {
	...
	if err != nil {
		if accessbinding.IsStatusWithCode(err, codes.NotFound) {
			tflog.Debug(ctx, fmt.Sprintf("Resource yandex_iam_workload_identity_oidc_federation_iam_binding '%s' not found, removing from state", u.federationId))
	...
	diag.Append(resp.SetAttribute(ctx, path.Root("id"), u.federationId+"/"+role.ValueString())...)
}
