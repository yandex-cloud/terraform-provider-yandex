...
package yandex_iam_service_account_iam_binding
	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"role": schema.StringAttribute{
				MarkdownDescription: "The role that should be assigned. Only one yandex_iam_service_account_iam_binding can be used per role.",
				Required:            true,
			},
			...
			"service_account_id": schema.StringAttribute{
				MarkdownDescription: "The ID of the `service_account` to attach the policy to.",
				Required:            true,
			},
...
func (u *IAMUpdater) Initialize(ctx context.Context, state accessbinding.Extractable, diag *diag.Diagnostics) {
	...
	diag.Append(state.GetAttribute(ctx, path.Root("service_account_id"), &id)...)
	u.service_accountId = id.ValueString()
}

func (r *IAMUpdater) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "yandex_iam_service_account_iam_binding"
}
...
func (r *IAMUpdater) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("service_account_id"), idParts[0])...)
...
func (u *IAMUpdater) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if len(binding) == 0 {
		tflog.Debug(ctx,
			fmt.Sprintf(
				"Resource yandex_iam_service_account_iam_binding '%s' is missing or deleted, marking policy binding as deleted",
				u.service_accountId,
			),
		)
		return
...
// Because yandex_iam_service_account_iam_binding is authoritative for a single role and applies changes via SetAccessBindings
...
func (u *IAMUpdater) ModifyPlan(ctx context.Context, req resource.ModifyPlanRequest, resp *resource.ModifyPlanResponse) {
	...
	if err != nil {
		...
		tflog.Debug(ctx, fmt.Sprintf("ModifyPlan: unable to read access bindings of yandex_iam_service_account_iam_binding '%s': %s", u.service_accountId, err))
		return
	}
	...
	resp.Diagnostics.AddWarning(
		fmt.Sprintf("Role %q will be removed from %d subject(s) of yandex_iam_service_account_iam_binding %q", role.ValueString(), len(removed), u.service_accountId),
...
func (u *IAMUpdater) GetResourceIamPolicy(ctx context.Context) (*accessbinding.Policy, error) {
	...
	for {
		md := new(metadata.MD)
		resp, err := iamv1sdk.NewServiceAccountClient(u.providerConfig.SDKv2).ListAccessBindings(ctx, &access.ListAccessBindingsRequest{
			ResourceId: u.service_accountId,
		...
		if traceHeader := md.Get("x-server-trace-id"); len(traceHeader) > 0 {
			tflog.Debug(ctx, "List yandex_iam_service_account_iam_binding trace header", map[string]interface{}{
				"x-server-trace-id": traceHeader[0],
			})
		}
		if traceHeader := md.Get("x-server-request-id"); len(traceHeader) > 0 {
			tflog.Debug(ctx, "List yandex_iam_service_account_iam_binding request header", map[string]interface{}{
...
func (u *IAMUpdater) SetResourceIamPolicy(ctx context.Context, policy *accessbinding.Policy) error {
	...
	op, err := iamv1sdk.NewServiceAccountClient(u.providerConfig.SDKv2).SetAccessBindings(ctx, req, grpc.Header(md))
	if err != nil {
		return fmt.Errorf("error setting access bindings of yandex_iam_service_account_iam_binding '%s': %w", u.service_accountId, err)
	}
...
func (u *IAMUpdater) UpdateResourceIamPolicy(ctx context.Context, policy *accessbinding.PolicyDelta) error {
	...
	for i := 0; i < accessbinding.CountBatches(dLen, bSize); i++ {
		...
		op, err := iamv1sdk.NewServiceAccountClient(u.providerConfig.SDKv2).UpdateAccessBindings(ctx, req)
		if err != nil {
			return fmt.Errorf("error updating access bindings of yandex_iam_service_account_iam_binding '%s': %w", u.service_accountId, err)
		}
...
func (u *IAMUpdater) refreshBindingState(ctx context.Context, req accessbinding.Extractable, resp accessbinding.Settable, diag *diag.Diagnostics) {
	...
	tflog.Debug(ctx, fmt.Sprintf("Retrieved access bindings of yandex_iam_service_account_iam_binding '%s': %+v", u.service_accountId, policy))
	...
	diag.Append(resp.SetAttribute(ctx, path.Root("service_account_id"), u.service_accountId)...)
	var sleep types.Int64
	req.GetAttribute(ctx, path.Root("sleep_after"), &sleep)
	diag.Append(resp.SetAttribute(ctx, path.Root("sleep_after"), sleep)...)
	diag.Append(resp.SetAttribute(ctx, path.Root("id"), u.service_accountId+"/"+role.ValueString())...)
}
...
func (u *IAMUpdater) iamPolicyReadModifySet(ctx context.Context, updater accessbinding.ResourceIamUpdater, modify iamPolicyModifyFunc) error {
	mutexKV.Lock(fmt.Sprintf("yandex_iam_service_account_iam_binding-%s", u.service_accountId))
	defer mutexKV.Unlock(fmt.Sprintf("yandex_iam_service_account_iam_binding-%s", u.service_accountId))
	tflog.Debug(ctx, fmt.Sprintf("Retrieving access bindings for yandex_iam_service_account_iam_binding '%s'", u.service_accountId))

	p, err := updater.GetResourceIamPolicy(ctx)
	if err != nil {
		return err
	}
	tflog.Debug(ctx, fmt.Sprintf("Retrieved access bindings for yandex_iam_service_account_iam_binding '%s': %+v\n", u.service_accountId, p))

	err = modify(p)
	if err != nil {
		return err
	}
	tflog.Debug(ctx, fmt.Sprintf("Setting access bindings for yandex_iam_service_account_iam_binding '%s' to %+v", u.service_accountId, p))

	err = updater.SetResourceIamPolicy(ctx, p)
	if err != nil {
		return fmt.Errorf("Error applying access bindings to yandex_iam_service_account_iam_binding '%s': %w", u.service_accountId, err)
	}
	tflog.Debug(ctx, fmt.Sprintf("Set policy for yandex_iam_service_account_iam_binding '%s'", u.service_accountId))
