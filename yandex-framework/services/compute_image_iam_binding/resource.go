...
package compute_image_iam_binding
...
func (u *IAMUpdater) GetResourceIamPolicy(ctx context.Context) (*accessbinding.Policy, error) {
	bindings, err := u.getAccessBindings(ctx, u.ImagegroupId)
	if err != nil {
		return nil, err
	}
...
func (u *IAMUpdater) getAccessBindings(ctx context.Context, id string) ([]*access.AccessBinding, error) {
	...
	for {
		resp, err := u.ProviderConfig.SDK.Compute().Image().ListAccessBindings(ctx, &access.ListAccessBindingsRequest{
			ResourceId: id,
...
func (u *IAMUpdater) SetResourceIamPolicy(ctx context.Context, policy *accessbinding.Policy) error {
	...
	op, err := u.ProviderConfig.SDK.WrapOperation(u.ProviderConfig.SDK.Compute().Image().SetAccessBindings(ctx, req))
	if err != nil {
...
func (u *IAMUpdater) UpdateResourceIamPolicy(ctx context.Context, policy *accessbinding.PolicyDelta) error {
	...
	for i := 0; i < accessbinding.CountBatches(dLen, bSize); i++ {
		...
		op, err := u.ProviderConfig.SDK.WrapOperation(u.ProviderConfig.SDK.Compute().Image().UpdateAccessBindings(ctx, req))
		if err != nil {
...
func (u *IAMUpdater) GetMutexKey() string {
	return fmt.Sprintf("iam-compute-imageGroup-%s", u.ImagegroupId)
}

func (u *IAMUpdater) DescribeResource() string {
	return fmt.Sprintf("compute-imageGroup '%s'", u.ImagegroupId)
}

func (u *IAMUpdater) GetNameSuffix() string {
	return "compute_image_iam_binding"
}

func (u *IAMUpdater) GetSchemaAttributes() map[string]schema.Attribute {
	return map[string]schema.Attribute{
		u.GetIdAlias(): schema.StringAttribute{
			MarkdownDescription: "The ID of the compute Image to attach the policy to.",
			Required:            true,
		},
	}
}
...
func (u *IAMUpdater) Initialize(ctx context.Context, state accessbinding.Extractable, diag *diag.Diagnostics) {
	...
	diag.Append(state.GetAttribute(ctx, path.Root("image_id"), &id)...)
	u.ImagegroupId = id.ValueString()
}
