...
// Map `terraformEntityId` -> mysql.HostSpec
...
func hostsDiff(planHosts map[string]Host, apiHosts map[string]Host) (toCreate map[string]*mysql.HostSpec, toUpdate []*mysql.UpdateHostSpec, toDelete []string) {
	toCreate = make(map[string]*mysql.HostSpec)
	toUpdate = []*mysql.UpdateHostSpec{}
	toDelete = []string{}
	...
	for tfid, p := range planHosts {
		if apiHost, exist := apiHosts[tfid]; !exist {
			...
			if p.AssignPublicIp != apiHost.AssignPublicIp || p.ReplicationSource != apiHost.ReplicationSource {
				...
				toUpdate = append(toUpdate, &mysql.UpdateHostSpec{
...
func (h *hostsSquats) Step1(ctx context.Context, hostSpecs basetypes.MapValue) ([]*mysql.HostSpec, diag.Diagnostics) {
	var diag diag.Diagnostics
	hostSpecsSlice := []*mysql.HostSpec{}
	mapping := map[hostEntity][]string{} // We will use this slice later to initialize the state

	hostSpecsMap, diags := hostsFromMapValue(ctx, hostSpecs) // Map `terraformEntityId` -> mysql.HostSpec
	diag.Append(diags...)
	if diags.HasError() {
		return nil, diags
	}
	...
	for tfid, spec := range hostSpecsMap {
		hostSpecsSlice = append(hostSpecsSlice, &mysql.HostSpec{
