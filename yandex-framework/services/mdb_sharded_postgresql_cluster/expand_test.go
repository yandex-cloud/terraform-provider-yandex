...
func TestYandexProvider_MDBSPQRClusterConfigAccessExpand(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		testname      string
		reqVal        types.Object
		expectedVal   *spqr.Access
		expectedError bool
	}{
		{
			"CheckAllAttributes",
			buildTestAccessObj(&trueAttr, &trueAttr, &trueAttr, &trueAttr),
			&spqr.Access{
				DataLens:     true,
				WebSql:       true,
				DataTransfer: true,
		...
		{
			"CheckPartlyAttributes",
			buildTestAccessObj(&trueAttr, &falseAttr, nil, nil),
			&spqr.Access{
				DataLens:     true,
				DataTransfer: false,
				WebSql:       false,
		...
		{
			"CheckWithoutAttributes",
			buildTestAccessObj(nil, nil, nil, nil),
			&spqr.Access{},
			false,
		},
		{
			"CheckNullAccess",
			types.ObjectNull(AccessAttrTypes),
			&spqr.Access{},
			false,
		},
		//{
		//	"CheckRandomAttributes",
	...
	for _, c := range cases {
		...
		res := expandAccess(ctx, c.reqVal, diags)
