...
package compute_disk_iam_binding
...
func (u *IAMUpdater) GetResourceIamPolicy(ctx context.Context) (*accessbinding.Policy, error) {
	bindings, err := u.getAccessBindings(ctx, u.DiskId)
	if err != nil {
		return nil, err
	}
...
func (u *IAMUpdater) getAccessBindings(ctx context.Context, id string) ([]*access.AccessBinding, error) {
	...
	for {
		resp, err := u.ProviderConfig.SDK.Compute().Disk().ListAccessBindings(ctx, &access.ListAccessBindingsRequest{
			ResourceId: id,
...
func (u *IAMUpdater) SetResourceIamPolicy(ctx context.Context, policy *accessbinding.Policy) error {
	...
	op, err := u.ProviderConfig.SDK.WrapOperation(u.ProviderConfig.SDK.Compute().Disk().SetAccessBindings(ctx, req))
	if err != nil {
...
func (u *IAMUpdater) UpdateResourceIamPolicy(ctx context.Context, policy *accessbinding.PolicyDelta) error {
	...
	for i := 0; i < accessbinding.CountBatches(dLen, bSize); i++ {
		...
		op, err := u.ProviderConfig.SDK.WrapOperation(u.ProviderConfig.SDK.Compute().Disk().UpdateAccessBindings(ctx, req))
		if err != nil {
...
func (u *IAMUpdater) GetMutexKey() string {
	return fmt.Sprintf("iam-compute-disk-%s", u.DiskId)
}

func (u *IAMUpdater) DescribeResource() string {
	return fmt.Sprintf("compute-disk '%s'", u.DiskId)
}

func (u *IAMUpdater) GetNameSuffix() string {
	return "compute_disk_iam_binding"
}
...
func (u *IAMUpdater) Initialize(ctx context.Context, state accessbinding.Extractable, diag *diag.Diagnostics) {
	...
	diag.Append(state.GetAttribute(ctx, path.Root("disk_id"), &id)...)
	u.DiskId = id.ValueString()
}
