...
func ComputedResourceAttributes() map[string]schema.Attribute {
	attributes := ResourceAttributes()
	computed := make(map[string]schema.Attribute, len(attributes))
	...
	for name, attribute := range attributes {
		...
		case schema.SetAttribute:
			...
			computed[name] = a
		default:
			panic(fmt.Sprintf("usersettings: unsupported attribute type %T of the %q setting", attribute, name))
		}
	}
