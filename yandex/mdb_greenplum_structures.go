...
func expandGreenplumAccess(d *schema.ResourceData) *greenplum.Access {
	...
	if v, ok := d.GetOk("access.0.trino"); ok {
		out.Trino = v.(bool)
	}
...
func expandGreenplumUpdateMask(d *schema.ResourceData) *field_mask.FieldMask {
	mdbGreenplumUpdateFieldsMap := map[string]string{
		...
		"access.0.trino":           "config.access.trino",
...
func expandGreenplumConfigSpecDBMSConfig(resourceData *schema.ResourceData) (*greenplum.DBMSConfig, []string, error) {
	...
	// However, versions prior to 6.25 stored in separated fields and now deprecated and unsupported
	switch {
