...
func resourceYandexYDBDatabaseDedicated() *schema.Resource {
	return &schema.Resource{
		...
		Delete: resourceYandexYDBDatabaseDedicatedDelete,
...
func resourceYandexYDBDatabaseDedicatedDelete(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)

	ctx, cancel := config.ContextWithTimeout(d.Timeout(schema.TimeoutDelete))
	defer cancel()

	req := ydb.DeleteDatabaseRequest{
		DatabaseId: d.Id(),
	}

	op, err := config.sdk.YDB().Database().Delete(ctx, &req)
...
func resourceYandexYDBDatabaseDedicatedRead(d *schema.ResourceData, meta interface{}) error {
	...
	database, err := config.sdk.YDB().Database().Get(config.Context(), &ydb.GetDatabaseRequest{
		DatabaseId: d.Id(),
	})

	if err != nil {
		return handleNotFoundError(err, d, fmt.Sprintf("YDB Database %q", d.Get("name").(string)))
	}
...
func flattenYandexYDBDatabaseDedicated(d *schema.ResourceData, database *ydb.Database) error {
	...
	return flattenYandexYDBDatabaseCommon(d, database)
}

func flattenYandexYDBDatabaseCommon(d *schema.ResourceData, database *ydb.Database) error {
