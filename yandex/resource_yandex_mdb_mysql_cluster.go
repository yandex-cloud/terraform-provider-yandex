						...
						"permission": {
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"roles": {
										Type:        schema.TypeList,
										Description: "List user's roles in the database. Allowed roles: `ALL`,`ALTER`,`ALTER_ROUTINE`,`CREATE`,`CREATE_ROUTINE`,`CREATE_TEMPORARY_TABLES`, `CREATE_VIEW`,`DELETE`,`DROP`,`EVENT`,`EXECUTE`,`INDEX`,`INSERT`,`LOCK_TABLES`,`SELECT`,`SHOW_VIEW`,`TRIGGER`,`UPDATE`,`REFERENCES`.",
										Elem: &schema.Schema{
											Type: schema.TypeString,
										},
										Optional: true,
...
var mdbMysqlUpdateFieldsMap = map[string]string{
	...
	"access.0.data_lens":                "config_spec.access.data_lens",
	"access.0.web_sql":                  "config_spec.access.web_sql",
	"access.0.data_transfer":            "config_spec.access.data_transfer",
	"access.0.yandex_query":             "config_spec.access.yandex_query",
	...
	"performance_diagnostics.0.enabled": "config_spec.performance_diagnostics.enabled",
	"performance_diagnostics.0.sessions_sampling_interval":   "config_spec.performance_diagnostics.sessions_sampling_interval",
	"performance_diagnostics.0.statements_sampling_interval": "config_spec.performance_diagnostics.statements_sampling_interval",
