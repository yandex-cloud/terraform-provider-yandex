...
func TestFlattenInstanceGroupVariable(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "test1",
			v:        instancegroup.Variable{Key: "test_key", Value: "test_value"},
			expected: map[string]interface{}{"key": "test_key", "value": "test_value"},
		},
	}
...
func TestExpandInstanceGroupVariables(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "soft",
			v:        append(make([]interface{}, 0), map[string]interface{}{"key": "test_key", "value": "test_value"}),
