...
const (
	mongodbRestoreBackupId          = "c9qvb4o0gnrh8ene82l7:mdb948pf5jgli93dbujp"
	mongodbRestoreBackupIdEncrypted = "c9q1omobes6h1t4bmm5b:mdbos8dlgnfe5nqvt5nl"

	mongodbResourceType = "yandex_mdb_mongodb_cluster"
	mongodbResourceFoo  = mongodbResourceType + ".foo"

	mongodbVPCDependencies = `
...
`
	...
	resourceYandexMdbMongodbClusterTemplateText = mongodbVPCDependencies + `
...
func init() {
	resource.AddTestSweepers(mongodbResourceType, &resource.Sweeper{
		Name: mongodbResourceType,
...
func mdbMongoDBClusterImportStep() resource.TestStep {
	return resource.TestStep{
		ResourceName:      mongodbResourceFoo,
		ImportState:       true,
		...
		Steps: []resource.TestStep{
			// Create MongoDB Cluster
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(mongodbResourceFoo, "name", clusterName),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "folder_id", folderID),
					//todo set compatibility_version doesn't work in create method , change test after fix in api
					resource.TestCheckResourceAttr(mongodbResourceFoo, "cluster_config.0.feature_compatibility_version", "8.0"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "cluster_config.0.access.0.data_lens", "true"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "cluster_config.0.access.0.data_transfer", "true"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "cluster_config.0.backup_retain_period_days", "10"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "cluster_config.0.performance_diagnostics.0.enabled", "true"),
					...
					testAccCheckMDBMongoDBClusterHasDatabases(mongodbResourceFoo, []string{"testdb"}),
					testAccCheckMDBMongoDBClusterHasUsers(mongodbResourceFoo, map[string][]string{"john": {"testdb"}}),
					testAccCheckMDBMongoDBClusterContainsLabel(&r, "test_key", "test_value"),
					testAccCheckCreatedAtAttr(mongodbResourceFoo),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "security_group_ids.#", "1"),
					...
					resource.TestCheckResourceAttr(mongodbResourceFoo, "deletion_protection", "true"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "host.0.assign_public_ip", "false"),
			...
			// uncheck 'deletion_protection'
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &r, 2),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "deletion_protection", "false"),
				),
			},
			mdbMongoDBClusterImportStep(),
			...
			mdbMongoDBClusterImportStep(),
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(mongodbResourceFoo, "folder_id", folderID),
					testAccCheckMDBMongoDBClusterHasMongodSpec(&r, map[string]interface{}{"Resources": &s2Micro16hdd}),
					testAccCheckMDBMongoDBClusterHasUsers(mongodbResourceFoo, map[string][]string{"john": {"admin"}}),
					testAccCheckMDBMongoDBClusterHasDatabases(mongodbResourceFoo, []string{"testdb"}),
					testAccCheckCreatedAtAttr(mongodbResourceFoo),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "security_group_ids.#", "2"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "maintenance_window.0.type", "ANYTIME"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "cluster_config.0.backup_retain_period_days", "20"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "cluster_config.0.feature_compatibility_version", "7.0"),
				),
			},
			mdbMongoDBClusterImportStep(),
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &r, 4),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "name", clusterName+"-changed"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "folder_id", folderID),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "description", "Updated MongDB cluster"),
					resource.TestCheckResourceAttrSet(mongodbResourceFoo, "host.0.name"),
					...
					testAccCheckMDBMongoDBClusterHasUsers(mongodbResourceFoo, map[string][]string{"john": {"admin"}, "mary": {"newdb", "admin"}}),
					testAccCheckMDBMongoDBClusterHasDatabases(mongodbResourceFoo, []string{"testdb", "newdb"}),
					testAccCheckCreatedAtAttr(mongodbResourceFoo),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "security_group_ids.#", "1"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "maintenance_window.0.type", "WEEKLY"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "maintenance_window.0.day", "FRI"),
			...
			// Check if description can be set to null
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &r, 4),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "description", ""),
				),
			},
			mdbMongoDBClusterImportStep(),
			...
			// web_sql enable
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &r, 4),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "cluster_config.0.access.0.web_sql", "true"),
				),
			},
			mdbMongoDBClusterImportStep(),
		},
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create
			{
				Config: makeConfig(t, &configData, nil),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &testCluster, 2),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "name", clusterName),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "folder_id", folderID),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create
			{
				Config: makeConfig(t, &configData, nil),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &testCluster, 7),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "name", clusterName),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "folder_id", folderID),
					testAccCheckMDBMongoDBClusterHasRightVersion(&testCluster, version),
					testAccCheckMDBMongoDBClusterHasMongodSpec(&testCluster, map[string]interface{}{
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create
			{
				Config: makeConfig(t, &configData, nil),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &testCluster, 5),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "name", clusterName),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "folder_id", folderID),
					testAccCheckMDBMongoDBClusterHasRightVersion(&testCluster, version),
					testAccCheckMDBMongoDBClusterHasMongodSpec(&testCluster, map[string]interface{}{
		...
		Steps: []resource.TestStep{
			// Create
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckCreatedAtAttr(mongodbResourceFoo),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "security_group_ids.#", "1"),
					resource.TestCheckNoResourceAttr(mongodbResourceFoo,
		...
		Steps: []resource.TestStep{
			// Create
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(mongodbResourceFoo, "sharded", "false"),
				),
			},
			mdbMongoDBClusterImportStep(),
			// EnableSharding
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckMDBMongoDBClusterHasShards(mongodbResourceFoo, []string{"rs01"}),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "sharded", "true"),
				),
			},
			mdbMongoDBClusterImportStep(),
			// delete and add shard
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &testCluster, 4),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "name", clusterName),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "folder_id", folderID),
					testAccCheckMDBMongoDBClusterHasRightVersion(&testCluster, version),
					testAccCheckMDBMongoDBClusterHasMongodSpec(&testCluster, map[string]interface{}{
					...
					testAccCheckMDBMongoDBClusterHasShards(mongodbResourceFoo, []string{"rs02"}),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "sharded", "true"),
				),
			},
			mdbMongoDBClusterImportStep(),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckMDBMongoDBClusterHasRightVersion(&r, configData["Version"].(string)),
					testAccCheckMDBMongoDBClusterHasMongodSpec(&r, map[string]interface{}{"Resources": &s2Micro16hdd}),
					testAccCheckMDBMongoDBClusterHasDatabases(mongodbResourceFoo, []string{"db1"}),
					testAccCheckMDBMongoDBClusterHasUsers(mongodbResourceFoo, map[string][]string{"user1": {"db1", "admin"}}),
					testAccCheckMDBMongoDBClusterContainsLabel(&r, "test_key", "test_value"),
					testAccCheckCreatedAtAttr(mongodbResourceFoo),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "security_group_ids.#", "1"),
					resource.TestCheckResourceAttr(mongodbResourceFoo, "maintenance_window.0.type", "ANYTIME"),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create MongoDB Cluster with disk encryption
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &r, 1),
					resource.TestCheckResourceAttrSet(mongodbResourceFoo, "disk_encryption_key_id"),
				),
			},
			{
				ResourceName:            mongodbResourceFoo,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMDBMongoDBClusterExists(mongodbResourceFoo, &r, 1),
					resource.TestCheckNoResourceAttr(mongodbResourceFoo, "disk_encryption_key_id"),
				),
			},
		},
	})
...
func testAccCheckMDBMongoDBClusterDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != mongodbResourceType {
			continue
		}
