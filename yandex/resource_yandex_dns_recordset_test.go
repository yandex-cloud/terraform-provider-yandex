		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// Assert state holds the canonical joined form (no `" "` boundary).
					resource.TestCheckTypeSetElemAttr("yandex_dns_recordset.rs1", "data.*", longDKIMTXTValue),
				),
			},
			{
				Config:             testAccDNSRecordSetLongTXT(dnsZoneName, dnsZoneDescription, recordName, longDKIMTXTValue),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckTypeSetElemAttr("yandex_dns_recordset.rs1", "data.*", longDKIMTXTValue2),
				),
			},
			{
				Config:             testAccDNSRecordSetLongTXT(dnsZoneName, dnsZoneDescription, recordName, longDKIMTXTValue2),
