...
// Test that encrypted PostgreSQL Cluster can be created and destroyed
func TestAccMDBPostgreSQLCluster_diskEncryption_create(t *testing.T) {
	t.Parallel()

	version := postgresql_versions[rand.Intn(len(postgresql_versions))]
	log.Printf("TestAccMDBPostgreSQLCluster_diskEncryption_create: version %s", version)
	var cluster postgresql.Cluster
	clusterName := acctest.RandomWithPrefix("tf-postgresql-disk-encryption-create")
...
func TestAccMDBPostgreSQLCluster_dropDiskEncryption(t *testing.T) {
	...
	version := postgresql_versions[rand.Intn(len(postgresql_versions))]
	log.Printf("TestAccMDBPostgreSQLCluster_dropDiskEncryption: version %s", version)
	var cluster postgresql.Cluster
...
func TestAccMDBPostgreSQLCluster_addDiskEncryption(t *testing.T) {
	...
	version := postgresql_versions[rand.Intn(len(postgresql_versions))]
	log.Printf("TestAccMDBPostgreSQLCluster_addDiskEncryption: version %s", version)
	var cluster postgresql.Cluster
...
func clusterSettings(cluster *postgresql.Cluster, version string) (*clusterSettingsResult, error) {
	...
	case "18":
		userConfig := cluster.Config.GetPostgresqlConfig_18().UserConfig
		sharedPreloadLibraries := []int32{}
		for _, v := range userConfig.SharedPreloadLibraries {
			sharedPreloadLibraries = append(sharedPreloadLibraries, int32(v.Number()))
		}
	...
	case "18-1c":
		userConfig := cluster.Config.GetPostgresqlConfig_18_1C().UserConfig
		sharedPreloadLibraries := []int32{}
		for _, v := range userConfig.SharedPreloadLibraries {
			sharedPreloadLibraries = append(sharedPreloadLibraries, int32(v.Number()))
		}
...
func testAccMDBPGClusterDiskEncrypted(name, desc, environment, version string, diskSize int32) string {
